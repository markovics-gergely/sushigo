...
const envVariables = ['baseUrl', 'grant_type', 'client_id', 'client_secret', 'scope']
const envValues = Object.keys(process.env).filter(v => envVariables.includes(v)).map((key) => `    ${key}: '${process.env[key]}'`).join(',\r');
